...
if (hamburger && navMenu) {
    ...
    const apiKey = 'AIzaSyBDL-8Jdn94fWHcfER0Qzde-dJ2kFp72R8'; // LA TUA CHIAVE API
    const mapIframe = document.getElementById('googleMap');
    const mapContainer = document.getElementById('mapContainer');
    const mapPlaceholderOverlay = document.getElementById('mapPlaceholderOverlay');

    if (mapIframe && mapContainer && mapPlaceholderOverlay) {
        // Sostituisci il placeholder della chiave API nell'URL dell'iframe
        mapIframe.src = mapIframe.src.replace('MAP_API_KEY_PLACEHOLDER', apiKey);

        // Non è più necessario JavaScript per l'hover, gestito da CSS.
        // L'opacità iniziale e pointer-events sono gestiti da CSS.
    } else {
        console.error('Elementi della mappa non trovati. Assicurati che gli ID siano corretti nellHTML.');
    }
