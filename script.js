...
// Funzione per animare i numeri delle statistiche
function animateNumbers() {
    ...
    statNumbers.forEach(stat => {
        ...
        const duration = 2000; // Durata animazione in millisecondi
...
function openModal(imageSrc, imageAlt) {
    ...
    // Aggiunge gli event listeners per il touch swipe
...
function loadCurrentImage() {
    ...
    // Aggiorna i pulsanti di navigazione e gli indicatori
    updateNavigationButtons();
    updateModalIndicators();
}

// Crea gli indicatori a pallini per il modale
...
function handleTouchMove(e) {
    ...
    // Previene il comportamento di default solo se è uno swipe orizzontale
...
function handleTouchEnd(e) {
    ...
    // Verifica se è uno swipe orizzontale valido
...
function closeModal() {
    ...
    // Rimuove gli event listeners per il touch
    if (modalImage) {
...
if (hamburger && navMenu) {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            // Special handling for "Richiedi Preventivo" button on mobile
            if (targetId === '#contact' && this.classList.contains('btn-primary')) {
                // Check if we're on mobile (screen width <= 768px)
                if (window.innerWidth <= 768) {
                    // On mobile, scroll to the "Richiedi un Preventivo" heading instead
                    const preventiveHeading = document.querySelector('.contact-form h3');
                    if (preventiveHeading && preventiveHeading.textContent.includes('Richiedi un Preventivo')) {
                        targetElement = preventiveHeading;
                    }
                }
            }
...
${message}
    ...
    // Inizializza l'animazione delle statistiche
    initStatsAnimation();
